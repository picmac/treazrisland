...
import { getRequestUserId } from '../modules/roms/rom.controller';
import type { SaveStateRecord } from '../modules/roms/save-state.service';
import type { FastifyError, FastifyPluginAsync, FastifyReply, FastifyRequest } from 'fastify';

const romIdSchema = z.union([z.string().uuid(), z.string().cuid(), z.string().min(1)]);
...
export const saveStateRoutes: FastifyPluginAsync = async (fastify) => {
