...
`scripts/dev-http.sh` helper now understands bind-address overrides so you can run both apps directly on the
self-hosted runner without touching the production compose files:

1. Copy `/opt/treazrisland/app/.env.example` to `/opt/treazrisland/config/dev-http.env` and keep
   `TREAZ_TLS_MODE=http`.
2. Export the LAN overrides before starting the helper. Replace `192.168.50.10` with the runner's IP on your
   trusted network.
...
If you skip the host overrides, the helper attempts to advertise the runner's primary LAN IP automatically, but explicit values
avoid surprises when multiple interfaces exist.
