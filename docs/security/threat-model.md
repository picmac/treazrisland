...
1. **Authentication & Authorization**
 ...
 - [x] Auth rate limits rehearsed and logged. _(Manual k6 drill captured 2025-02-22 in `docs/security/reports/2025-02-22-rate-limit-drill.md` with complementary Vitest assertions tracked in `docs/security/reports/2025-02-24-auth-hardening.md`. Aligns with hardening checklist item **SEC-45**.)_
  - [x] Initial admin bootstrap is single-use and rate limited. _(Fastify onboarding route `/onboarding/admin` rejects once a user exists, enforces `RATE_LIMIT_AUTH_*` thresholds, and records status in `SetupState`. A startup helper now honors `TREAZ_BOOTSTRAP_ADMIN_*` credentials to seed the first admin automatically, hashing with Argon2 and marking the setup step before traffic is served; regression coverage in `backend/tests/onboarding/onboarding.test.ts` and `backend/tests/setup/bootstrap-admin.test.ts`. Operators must rotate or delete the bootstrap variables immediately after the first run.)_
