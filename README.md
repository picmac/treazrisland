...
| Frontend security | `TREAZ_TLS_MODE` | Environment templates now default to `auto`, which enforces HTTPS when `TREAZ_RUNTIME_ENV` resolves to production and relaxes to HTTP on LAN/dev builds. Override to `https` to harden previews or `http` to force plaintext for diagnostics. |
