...
export async function loginWithPassword(page: Page, credentials: LoginCredentials = defaultCredentials) {
  ...
  const tokenHandle = await page.waitForFunction(() => window.localStorage.getItem('treazr.accessToken'));
